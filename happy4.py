...
def try_connect(family, ip, port, timeout_ms, hostname):
    ...
    try:
        ...
        return {"ip": ip, "port": port, "duration_ms": duration, "success": True, "socket": s, "family": family, "hostname": hostname}
...
def resolve_addresses(host: str, port: int, family) -> List[Tuple[int, str]]:
    try:
    ...
    with ThreadPoolExecutor(max_workers=len(all_addresses)) as executor:
        ...
        for future in as_completed(future_map):
            ...
            if res['success'] and time_taken <= timeout and not winner:
                winner = AddrInfo(family=fam, ip=ip, port=port, duration_ms=time_taken, hostname=h)
                # Cancel remaining
                for f in future_map:
                    if not f.done():
                        f.cancel()
