...
def resolve_addresses(host: str, port: int, family, dns_timeout=DNS_TIMEOUT) -> List[Tuple[int, str]]:
    ...
    q = Queue()
    p = Process(target=resolver, args=(q,))
    p.start()
    p.join(timeout=dns_timeout)

    if p.is_alive():
        p.terminate()
        p.join()
        logging.error(f"DNS resolution timed out for {host} after {dns_timeout} seconds.")
        return []

    if q.empty():
        logging.error(f"No response from DNS resolver process for {host}.")
        return []

    result = q.get()
    if isinstance(result, Exception):
        logging.error(f"DNS resolution failed for {host}: {result}")
        return []

    return [(fam, addr[0]) for fam, _, _, _, addr in result]
...
def happyeyeballs(
    ...
    with ThreadPoolExecutor(max_workers=len(all_addresses)) as executor:
        ...
        for future in as_completed(future_map):
            ...
            if res['success']:
                ...
                successes.append(AddrInfo(family=fam, ip=ip, port=port, duration_ms=time_taken, hostname=h))
            else:
    ...
    # Pick the fastest successful connection
    winner = min(successes, key=lambda x: x.duration_ms)
    return winner
